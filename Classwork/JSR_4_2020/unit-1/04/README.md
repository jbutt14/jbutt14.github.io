...
```javascript
// es6 arrow function 
let speak = (words) => {  // the curly brace opens a multiline function
  return console.log(words); // if we use curly braces, then we need an explicit "return"
};

// es6 arrow function source https://javascript.info/arrow-functions-basics
```
...
// RESULTS IN ERROR:
// TypeError: Cannot access 'speak' before initialization
```
